...
export default function CredibilitySection({ content }: { content?: any }) {
  const defaultContent = {
    headline: "Built For Growth At Any Stage",
    description: "Most centers are not failing because of tutoring quality. They are struggling with spreadsheet workflows, repetitive admin tasks, and inconsistent records that waste team time.",
    sub: "Thetix gives centers one centralized operating system with automation and tracking so teams can run cleanly at one location or across many.",
    painPoints: [
    ...
    <section className="relative py-24 md:py-32 overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-6 text-center">
        ...
        >
          ...
          {c.sub && (
            <p className="text-lg text-slate-700 font-medium leading-relaxed mb-16">{c.sub}</p>
          )}
        </motion.div>
        {/* Pain points */}
        <motion.div
        ...
        >
          {c.painPoints.map((point: string, i: number) => (
            <div
              key={i}
